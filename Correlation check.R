...
setwd("H:/Dropbox/Uni - Mathematics and Economics/ATP topic course/Rcode")
library(dataCleanFunc)
library(highfrequency)

data <- read.csv("masterData_TLT_UPDATED.csv", header = TRUE)

data2 <- read.csv("MasterData_SPY_UPDATED.csv", header = TRUE)

cleanData <- nanRemover(data)
...
timestamps <- timestamp(cleanData, dates)

timestamps2 <- timestamp(cleanData2, dates2)

timeseriesTLT <- timeseriesList(data, cleanData, timestamps)

timeseriesSPY <- timeseriesList(data2, cleanData2, timestamps2)



#Random day of returns:
timeseriesSPYret <- list()
timeseriesTLTret <- list()
for(i in 1:length(timeseriesSPY)){

	timeseriesSPYret[[i]] <- diff(log(timeseriesSPY[[i]]))[-1]
	timeseriesTLTret[[i]] <- diff(log(timeseriesTLT[[i]]))[-1]
...
for(i in 1:length(timeseriesSPY)){
	...
	aggregatespy[[i]] <- aggregatets(timeseriesSPYret[[i]], on="seconds",k=75)
	aggregatetlt[[i]] <- aggregatets(timeseriesTLTret[[i]], on="seconds",k=75)
...
acfnumbersspy <- matrix(0L, ncol = 250, nrow = 31)
acfnumberstlt <- matrix(0L, ncol = 250, nrow = 31)

for(i in 1:length(aggregatespy)){

acfnumbersspy[, i] <- acf(aggregatespy[[i]], lag.max  = 30, plot =  F)$acf
acfnumberstlt[, i] <- acf(aggregatetlt[[i]], lag.max  = 30, plot =  F)$acf
